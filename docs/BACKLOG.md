...
- **Refactor de archivos grandes — `SceneRenderer_Render.cpp` único pendiente** (F2H83 actualizó este item, 2026-05-23). F2H81 partió `AssetBrowserPanel.cpp` 924→395+Tabs, `EntitySerializer.cpp` 819→504+Parse, `Components.h` 936→agregador. F2H83 atacó los otros 2 diferidos: `EditorApplication.h` 835→673 (sessions a `_Sessions.inl`) y `EditorRenderPass_Overlay.cpp` 859→490 (5 overlays F1-debug a sibling `_Overlay_Debug.cpp`). **Queda solo `SceneRenderer_Render.cpp` (978 LOC)**: las 2 lambdas centrales (`applyShaderUniforms`, `drawMeshRenderer`) capturan 9+ locales compartidos entre todos los passes (instanced/static/skinned/brush/compiled-mesh/OIT); split limpio requiere un `FrameRenderContext` invasivo + promover lambdas a métodos privados + extraer cada pass con verificación visual. Sin tests visuales el riesgo de regresión silenciosa es alto. **Criterios para volver a evaluar**: (a) el archivo crece >1100 LOC, (b) emerge bug gráfico que requiere modificar 3+ passes, (c) se agrega cobertura golden-pixel al pipeline, (d) comienza rediseño del renderer (Vulkan/D3D12). Detalle en [`hitos/F2H83.md`](hitos/F2H83.md) decisión D3.
