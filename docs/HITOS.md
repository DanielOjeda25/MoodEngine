...
**🏁 23/44 hitos de Fase 2 hechos.** Próximo: **F2H25 — 4-viewport Hammer-style layout** (heredado, antes F2H22→F2H23→F2H24 candidato). Workspace nuevo "Hammer" con dockspace en 4 cuadrantes (1 perspectiva 3D + 3 ortográficas top/front/side en wireframe `glPolygonMode(GL_LINE)` + grid 2D + crosshair) + drag-edit entre vistas con grid snap. Tradeoff: ~4× costo CPU del frame con cuatro viewports — mitigar reusando shadow pass / light grid compartido entre cámaras. Reusa la infra de workspaces de F2H7+F2H22 — el Hammer entra como un workspace más con su propio `buildHammerWorkspace` en `Dockspace.cpp`.
