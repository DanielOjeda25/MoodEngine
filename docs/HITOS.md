...
- [x] **F2H10** — CI/CD con GitHub Actions (completado, tag `v1.1.7-fase2-hito10`). Bloques A-F. Pipeline de integración continua: `.github/workflows/build.yml` corre en cada push a `main` + cada PR (Windows MSVC + cmake preset + ctest), con cache agresivo de CPM deps (key = hashFiles `CMakeLists.txt`). Builds posteriores ~2-4 min. `.github/workflows/release.yml` con trigger en `v*.*.*` crea GitHub Release automático con el message del tag como body. Badge en README.md activo. **Sin Linux** (multiplataforma fuera de scope) y **sin Dependabot** (proyecto solo, deps pinneadas). Suite **396/6948** sin cambios — F2H10 es infraestructura. Bloques A-E. Cierra el stub de "Guardar como" pendiente desde Hito 6. **Backend ya estaba**: el `.moodproj` schema soporta `maps[]` y `defaultMap` desde el día 1; F2H8 implementa el UX faltante. Helper PURO `MapsManager` para mantener invariantes (>= 1 mapa, default + current dentro de la lista, dedup por `generic_string`). 5 handlers en `EditorApplication`: NewMap, SaveMapAs, OpenMap, SetCurrentMapAsDefault, DeleteCurrentMap. UI `Archivo > Mapa` con submenu de la lista de mapas (current marcado con `*`, default con `[default]`). **Bug fix también incluido**: durante testing visual emergió que `Floor` (no solo `Tile_X_Y`) se duplicaba al cargar mapas modificados — un Floor del rebuild + otro del JSON → 2 planos 48×48 superpuestos → fillrate brutal y freeze percibido. Fix extiende `SceneLoader::applyOneEntity` para reemplazar también entidades con tag `"Floor"`. Test del round-trip cubre el caso. Suite **367→396/6948** (+13 cases en `test_maps_manager.cpp` + casos del Floor). Bloques A-I. Tabs en la misma fila del menú principal (Archivo/Editar/Ver/Ayuda | Layout/Scripting/Profile/Materials | ... | Play). 4 workspaces default con layouts distintos via `ImGui::DockBuilder`: Layout (general), Scripting (script editor + console grandes), Profile (Performance HUD destacado), Materials (Material Editor + AssetBrowser destacados). Switching con auto-save del layout previo: `SaveIniSettingsToMemory`/`LoadIniSettingsFromMemory` aplicado ANTES de `ImGui::NewFrame` (no se permite dentro de un frame activo). Persistencia en `.moodproj` (sin back-compat — proyectos viejos borrados según el dev). Menú "Ver" reorganizado en submenús Scene/Assets/Debug/World; cada `IPanel` tiene `category()` virtual. Suite **358→367/6847** (+9 cases en `test_workspace_manager.cpp`). Bloques A-I. Retoma el LOD original (era F2H4 plan, postergado dos veces). Sigue el patrón industria estándar (Unity/Unreal/Godot) con cimiento sólido: persistencia con cache lateral al `.moodmap` (`assets/.cache/lods/<hash>.moodlod`), per-mesh `lodDistances` con default global, skinned saltea (re-mapping de bone weights = scope futuro). Nueva dep `meshoptimizer v0.21` via CPM. `MeshAsset` gana `lod1Submeshes`/`lod2Submeshes` + helper `selectLod(distance, thresholds)` + `submeshesForLod(lod)` con fallback automático. `LodCache` con FNV-1a hash + mtime/size validation. `MeshLoader::loadMeshWithAssimp` auto-genera LODs (50% / 15% tris) tras cargar el mesh, persiste cache. `groupByBatch` agrupa por (mesh, material, lod) calculando distancia camera→entidad. `SceneRenderer::renderScene` emite el LOD correcto vía `submeshesForLod`. Plots Tracy `PBR::InstancesLod{0,1,2}`. Inspector muestra info de LODs read-only. Suite **345→358/6814** (+12 cases en `test_lod.cpp` + 1 case en `test_render_batching.cpp` para BatchKey con lod). Validación de impacto medible queda postergada a cuando se carguen meshes complejos (Fox.glb / kenney_survival props) — los stress de cubos primitivos no se benefician (12 tris ya, no hay con qué reducir).
