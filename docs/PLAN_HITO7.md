...
- [ ] Bloque 5 del Hito 6 (`editor.state.json`): si empieza a molestar tener que reabrir el proyecto cada vez, arrancar por acá. Si no, seguir diferido.
- [ ] Diálogo de dirty-check (`pfd::message`) al Cerrar/Abrir/Nuevo cuando hay cambios sin guardar. Rápido y evita perder trabajo.

## Bloque 1 — EnTT como dependencia

- [ ] CPMAddPackage para `skypjack/entt` (última versión estable, v3.13.x).
- [ ] Linkearla a `MoodEditor` y a `mood_tests`.
- [ ] Compilar un smoke test (crear registry, entity, asignar un componente trivial, leerlo) para confirmar que engancha.

## Bloque 2 — Fachada Entity + Scene

- [ ] `src/engine/scene/Scene.{h,cpp}`: envuelve `entt::registry`. API pública:
...
- [ ] `src/engine/scene/Entity.{h,cpp}`: contiene `entt::entity m_handle` + `Scene* m_scene` (o `entt::registry*`). API:
  - `template<typename T, typename... Args> T& addComponent(Args&&...)`.
  - `template<typename T> T& getComponent()`.
  - `template<typename T> bool hasComponent() const`.
  - `template<typename T> void removeComponent()`.
  - `operator bool()` / `operator==` / `entt::entity handle()`.
- [ ] Esconder todo `entt::` fuera del impl: los headers del editor sólo incluyen `Scene.h` + `Entity.h`.

## Bloque 3 — Componentes básicos

- [ ] `src/engine/scene/Components.h` (todos en un header liviano):
...
- [ ] `EditorApplication` gana un `std::unique_ptr<Scene>` al lado del `GridMap` actual. El `GridMap` se mantiene por compatibilidad con el serializer y las colisiones del Hito 4; el paso real a "sólo entidades" es más adelante (el serializer debería convertir el grid en entidades al cargar, y las colisiones empezar a iterar AABBs por componente).
- [ ] `buildInitialTestMap()` además de llenar el `GridMap`, crea 29 entidades en la escena — una por tile sólido — con `TransformComponent` (posición = centro del tile, scale = tileSize) + `MeshRendererComponent { cubeMesh, texturaDelTile }`.
- [ ] `GridRenderer::draw` se reemplaza/complementa con un `RenderSystem::draw(Scene&, ...)` que itera entidades con `Transform+MeshRenderer`.

## Bloque 5 — Panel Hierarchy

- [ ] `src/editor/panels/HierarchyPanel.{h,cpp}`.
- [ ] Recibe `Scene*` vía setter. Itera entidades y las muestra como `ImGui::Selectable` con el nombre del `TagComponent`.
- [ ] Click selecciona la entidad. Guarda `Entity m_selected` compartido con el Inspector (via `EditorUI` o un `SelectionService` dedicado).
- [ ] Integrar al dockspace default: a la izquierda del Viewport, por encima del Asset Browser.

## Bloque 6 — Panel Inspector funcional

- [ ] `InspectorPanel` deja de ser placeholder. Recibe `Entity` seleccionada.
- [ ] Por cada componente conocido, dibuja sus campos con widgets ImGui:
...
- [ ] Marcar dirty al modificar cualquier valor (`markDirty()` del Hito 6).

## Bloque 7 — Tests

- [ ] `tests/test_scene.cpp`: crear scene, agregar entity, check que `createEntity` setea `TagComponent` y `TransformComponent` por default.
- [ ] `tests/test_entity.cpp`: addComponent/getComponent/hasComponent/removeComponent para un componente trivial (p.ej. `struct Counter { int v; }`).

## Bloque 8 — Cierre

- [ ] Recompilar y verificar (build limpio + tests + editor arranca con Hierarchy poblado).
- [ ] Actualizar `docs/HITOS.md`, `docs/DECISIONS.md`, `docs/ESTADO_ACTUAL.md`.
- [ ] Commits atómicos en español.
- [ ] Tag `v0.7.0-hito7` + push.
- [ ] Crear `docs/PLAN_HITO8.md` (Lua scripting).
