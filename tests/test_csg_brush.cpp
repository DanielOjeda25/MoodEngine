...
TEST_CASE("buildBrushMesh: triangulos no degenerados (vertices distintos)") {
    const Brush b = makeBoxBrush(glm::mat4(1.0f));
    const BrushMeshData mesh = buildBrushMesh(b);
    REQUIRE(mesh.totalIndexCount() % 3 == 0);
    for (usize i = 0; i < mesh.submeshes[0].indices.size(); i += 3) {
        const u32 a = mesh.submeshes[0].indices[i];
        const u32 ib = mesh.submeshes[0].indices[i + 1];
        const u32 ic = mesh.submeshes[0].indices[i + 2];
        CHECK(a != ib);
        CHECK(ib != ic);
        CHECK(a != ic);
    }
...
TEST_CASE("buildBrushMesh: triangulos con area > 0") {
    ...
    const BrushMeshData mesh = buildBrushMesh(b);
    for (usize i = 0; i < mesh.submeshes[0].indices.size(); i += 3) {
        const glm::vec3& p0 = mesh.submeshes[0].vertices[mesh.submeshes[0].indices[i]].position;
        const glm::vec3& p1 = mesh.submeshes[0].vertices[mesh.submeshes[0].indices[i + 1]].position;
        const glm::vec3& p2 = mesh.submeshes[0].vertices[mesh.submeshes[0].indices[i + 2]].position;
...
TEST_CASE("buildBrushMesh: triangulos respetan la normal de la cara (CCW visto desde +n)") {
    ...
    const BrushMeshData mesh = buildBrushMesh(b);
    for (usize i = 0; i < mesh.submeshes[0].indices.size(); i += 3) {
        const auto& v0 = mesh.submeshes[0].vertices[mesh.submeshes[0].indices[i]];
        const auto& v1 = mesh.submeshes[0].vertices[mesh.submeshes[0].indices[i + 1]];
        const auto& v2 = mesh.submeshes[0].vertices[mesh.submeshes[0].indices[i + 2]];
...
TEST_CASE("buildBrushMesh: brush trasladado mueve la mesh consistentemente") {
    ...
    const Brush b = makeBoxBrush(T);
    const BrushMeshData mesh = buildBrushMesh(b);
    REQUIRE(mesh.totalVertexCount() == 24);
    for (const auto& v : mesh.submeshes[0].vertices) {
...
TEST_CASE("buildBrushMesh: determinismo bit-a-bit") {
    ...
    REQUIRE(m1.totalVertexCount() == m2.totalVertexCount());
    REQUIRE(m1.totalIndexCount() == m2.totalIndexCount());
    REQUIRE(m1.submeshes.size() == m2.submeshes.size());
    for (usize si = 0; si < m1.submeshes.size(); ++si) {
        const auto& s1 = m1.submeshes[si];
        const auto& s2 = m2.submeshes[si];
        for (usize i = 0; i < s1.vertices.size(); ++i) {
            CHECK(s1.vertices[i].position.x == s2.vertices[i].position.x);
            CHECK(s1.vertices[i].position.y == s2.vertices[i].position.y);
            CHECK(s1.vertices[i].position.z == s2.vertices[i].position.z);
        }
        for (usize i = 0; i < s1.indices.size(); ++i) {
            CHECK(s1.indices[i] == s2.indices[i]);
        }
    }
}
...
TEST_CASE("buildBrushMesh: las UV no son todas iguales (proyeccion planar funciona)") {
    ...
    f32 uMin = mesh.submeshes[0].vertices[0].uv.x, uMax = mesh.submeshes[0].vertices[0].uv.x;
    f32 vMin = mesh.submeshes[0].vertices[0].uv.y, vMax = mesh.submeshes[0].vertices[0].uv.y;
    for (const auto& v : mesh.submeshes[0].vertices) {
...
TEST_CASE("brushSubmeshToInterleaved: box -> 36 vertices * 11 floats = 396 floats") {
    const Brush b = makeBoxBrush(glm::mat4(1.0f));
    const BrushMeshData mesh = buildBrushMesh(b);
    REQUIRE(mesh.totalIndexCount() == 36);
    REQUIRE(mesh.submeshes.size() == 1);
    const std::vector<f32> interleaved =
        brushSubmeshToInterleaved(mesh.submeshes[0]);
    CHECK(interleaved.size() == 36 * 11);
}

TEST_CASE("brushSubmeshToInterleaved: layout pos(3)+color(3)+uv(2)+normal(3)") {
    const Brush b = makeBoxBrush(glm::mat4(1.0f));
    const BrushMeshData mesh = buildBrushMesh(b);
    REQUIRE(mesh.submeshes.size() == 1);
    const std::vector<f32> flat =
        brushSubmeshToInterleaved(mesh.submeshes[0]);
    REQUIRE(flat.size() >= 11);

    const u32 firstIdx = mesh.submeshes[0].indices[0];
    const auto& v0 = mesh.submeshes[0].vertices[firstIdx];
...
TEST_CASE("brushSubmeshToInterleaved: submesh vacio produce vector vacio") {
    BrushSubmeshData empty;
    const std::vector<f32> flat = brushSubmeshToInterleaved(empty);
    CHECK(flat.empty());
}
...
TEST_CASE("buildBrushMesh: uvScale=2 duplica las UVs vs scale=1") {
    ...
    REQUIRE(mesh1.totalVertexCount() == mesh2.totalVertexCount());
    REQUIRE(mesh1.submeshes.size() == mesh2.submeshes.size());
    // Para cada vertex, mesh2.uv = mesh1.uv * 2.
    for (usize si = 0; si < mesh1.submeshes.size(); ++si) {
        const auto& v1 = mesh1.submeshes[si].vertices;
        const auto& v2 = mesh2.submeshes[si].vertices;
        for (usize i = 0; i < v1.size(); ++i) {
            CHECK(v2[i].uv.x == doctest::Approx(v1[i].uv.x * 2.0f).epsilon(k_eps));
            CHECK(v2[i].uv.y == doctest::Approx(v1[i].uv.y * 2.0f).epsilon(k_eps));
        }
    }
}
...
TEST_CASE("buildBrushMesh: uvOffset desplaza las UVs") {
    ...
    for (usize si = 0; si < mesh1.submeshes.size(); ++si) {
        ...
        const auto& v2 = mesh2.submeshes[si].vertices;
        for (usize i = 0; i < v1.size(); ++i) {
            CHECK(v2[i].uv.x == doctest::Approx(v1[i].uv.x + off.x).epsilon(k_eps));
            CHECK(v2[i].uv.y == doctest::Approx(v1[i].uv.y + off.y).epsilon(k_eps));
        }
    }
}
...
TEST_CASE("buildBrushMesh: lockToWorld=true cambia UVs al transformar") {
    ...
    REQUIRE(mesh1.totalVertexCount() == mesh2.totalVertexCount());
    REQUIRE(mesh1.totalVertexCount() > 0);

    // Con lockToWorld, las UVs cambian segun la posicion world.
    // No requerimos que CADA vertex difiera, pero al menos uno debe.
    const auto v1 = mesh1.allVertices();
    const auto v2 = mesh2.allVertices();
    bool anyDifferent = false;
    for (usize i = 0; i < v1.size(); ++i) {
        if (std::fabs(v1[i].uv.x - v2[i].uv.x) > k_eps ||
            std::fabs(v1[i].uv.y - v2[i].uv.y) > k_eps) {
            anyDifferent = true;
            break;
        }
    }
...
TEST_CASE("buildBrushMesh: lockToWorld=false ignora worldMatrix") {
    ...
    const auto v2 = mesh2.allVertices();
    for (usize i = 0; i < v1.size(); ++i) {
        CHECK(v1[i].uv.x == doctest::Approx(v2[i].uv.x).epsilon(k_eps));
        CHECK(v1[i].uv.y == doctest::Approx(v2[i].uv.y).epsilon(k_eps));
    }
}
...
TEST_CASE("buildBrushMesh: box rotada produce mesh cerrada (centroide dentro de AABB)") {
    ...
    for (const auto& v : mesh.submeshes[0].vertices) centroid += v.position;
    centroid /= static_cast<f32>(mesh.submeshes[0].vertices.size());
