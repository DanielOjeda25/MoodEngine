...
void EditorApplication::drawEditorOverlay(ImDrawList* dl,
    ...
    m_scene->forEach<TransformComponent>(
        [&](Entity e, TransformComponent& t) {
            ...
            // F3H8 (bug reactivo) — labels de tag REMOVIDOS del overlay
            // de perspectiva. El dev pidio que los nombres aparezcan solo
            // en los ortho viewports del editor de mapas (mismo lugar
            // donde el toggle "Nombres" vive). El render de labels en
            // perspectiva inflaba el viewport con texto sobre cada
            // luz/audio/trigger/camera/particle — distrayente en escenas
            // pobladas. Los ortos siguen rendereando labels via
            // OrthoViewportPanel::onImGuiRender (gated por el mismo
            // m_showEntityLabels toggleable desde MapEditorTopBar).
        });
