...
// break-B3: ademas, el cuerpo unico (478 LOC, espagueti acotado) paso
// a 3 handlers privados (uno por sub-modo) para que el contexto de cada
// herramienta sea legible aislado.
//
...
// break-B3: dispatcher delgado. Cada sub-modo aplica sus propios guards
// (m_mode/m_scene/m_subMode/m_mapTool/sesiones) y es no-op si no aplica.
void EditorApplication::processOrthoToolModes() {
    handleOrthoBlockToolDrag();
    handleOrthoMarqueeSelect();
    handleOrthoVertexEdgeEdit();
}

// 2.4d) F2H29 Bloque C: block tool en orto. Activa cuando el drag arranca
// en empty space (sin brush bajo el cursor). Durante el drag dibuja un
// AABB cyan via debugRenderer (visible en perspectiva 3D); al soltar
// materializa un Box brush si las dims superan `m_hammerSnapStep` en
// ambos ejes del view. Snap aplica a las ESQUINAS (no al delta) -> brush
// queda alineado al grid. La altura sobre el eje perpendicular es
// default `snap * 4`.
//
// F2H30 Bloque B: solo aplica si subMode == Object — en Vertex/Edge mode
// no se crean brushes nuevos al drag.
// F2H30 Bloque C: tambien skipea si pincel poligonal activo (el block
// tool y el pincel comparten el LMB en empty space — el pincel tiene
// prioridad).
// F2H31 Bloque B: solo si m_mapTool == CreateBlock.
void EditorApplication::handleOrthoBlockToolDrag() {
    if (!(m_mode == EditorMode::Editor && m_scene &&
          m_subMode == EditorSubMode::Object &&
          !m_polyDraw.active &&
          m_mapTool == MapTool::CreateBlock &&
          m_orthoBlockSession.active)) return;
    ...
    // F2H31 Bloque C: snap-to-vertex toma prioridad si el toggle esta on;
    // sino fallback al grid. Snap se aplica a las ESQUINAS (no al delta)
    // para que el brush quede alineado al grid global o pegado al vertex
    // vecino.
    ...
    // Inflar el eje perpendicular del view a `snap * 4` centrado en 0.
    // forwardAxis tiene magnitud 1 en exactamente uno de los 3 ejes, los
    // otros 0.
    const f32 defaultHeight = (snap > 0.0f) ? (snap * 4.0f) : 4.0f;
    const glm::vec3 perpHalf = glm::abs(cam.forwardAxis())
                                * (defaultHeight * 0.5f);
    minPt -= perpHalf;
    maxPt += perpHalf;
    // Preview AABB cyan: lo encolamos en perspectiva (este drawAabb se
    // flushea con view+proj de la cam 3D en endFrame) Y guardamos los
    // puntos en la sesion para que EditorRenderPass los re-encole antes
    // de cada renderOrthoView (el flush adentro de renderOrthoView lo
    // dibuja con las matrices del orto).
    m_orthoBlockSession.previewMin = minPt;
    m_orthoBlockSession.previewMax = maxPt;
    m_orthoBlockSession.previewValid = true;
    if (m_sceneRenderer) {
        // F2H29 Bloque C: celeste GMod (mismo color del wireframe regular
        // en SceneRenderer_Ortho.cpp). Coherente con la paleta Valve+GMod
        // del workspace.
    ...
    if (ds.justEnded) {
        ...
        // Validar tamano minimo en los 2 ejes del plano de view (ignorar
        // eje perpendicular que ya tiene height default).
        ...
        if (widthOnRight >= minSize && widthOnUp >= minSize) {
...
// 2.4f) F2H31 Bloque B: marquee select en orto. Activa cuando el drag
// arranca en empty space + m_mapTool == Select. El rectangulo amarillo
// se dibuja desde EditorRenderPass leyendo `m_orthoMarquee.active` + el
// panel's dragState. Aca solo procesamos el cierre (justEnded): hit-test
// cada entidad con AABB world contra el rectangulo en ndc del orto;
// replace / Shift=add / Ctrl=toggle del SelectionSet.
void EditorApplication::handleOrthoMarqueeSelect() {
    if (!(m_mode == EditorMode::Editor && m_scene &&
          m_orthoMarquee.active &&
          m_mapTool == MapTool::Select)) return;
...
// 2.4e) F2H30 Bloque B: vertex/edge edit en orto. Activa cuando subMode
// == Vertex/Edge AND drag-down impacta un vertex o edge del brush ACTIVE
// (single brush — solo opera sobre activeSelection.active, no
// multi-edit). Mueve los planos incidentes via
// `d_new = d_old - dot(n_local, delta_local)` con
// `delta_local = R^-1 * delta_world`. Valida `isBrushValid` post; si
// rompe, revierte. Al soltar, pushea EditBrushGeometryCommand.
//
// F2H30 Bloque C fix: skipea si el pincel poligonal esta activo
// (togglePolygonDrawMode ya pone subMode=Object, pero la guarda extra
// previene que un session in-flight siga corriendo si el pincel se
// activo a mitad de drag).
void EditorApplication::handleOrthoVertexEdgeEdit() {
    if (!(m_mode == EditorMode::Editor && m_scene &&
          !m_polyDraw.active &&
          (m_subMode == EditorSubMode::Vertex ||
           m_subMode == EditorSubMode::Edge))) return;
    ...
    if (!m_orthoVertexEdit.active) {
        ...
        if (activeBrush && activeBrush.hasComponent<BrushComponent>()
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                // Hit. Capturar snapshot de planos + transform + pivot
                // local del vertex/edge.
                ...
                m_orthoVertexEdit.incidentPlanes = std::move(incident);
                m_orthoVertexEdit.tfPosBefore = tf.position;
                // Recompute pivot local (vertex pos para Vertex, midpoint
                // para Edge). Lo recomputamos del pick result en lugar de
                // cachearlo arriba para mantener el codigo simple.
                if (m_subMode == EditorSubMode::Vertex) {
                    auto vp = Csg::pickVertex(bc.brush, worldMat,
                        oView, oProj, ds.ndcStartX, ds.ndcStartY);
    ...
    if (!m_orthoVertexEdit.active) return;
    ...
    // F2H30 fix#1: snap se hace en WORLD space (no local). El grid del
    // workspace orto vive en world, no en local — si el brush tiene
    // tf.position != 0, snap en local queda desfasado. Pivot world =
    // tf.position + R * pivotLocalStart. Snap world pivot. Reconvertir
    // a delta_local via inverse(R).
    //
    // F2H30 fix#2: solo snap los ejes que el dev MUEVE
    // (|deltaWorld[i]| > eps). Snap todos los ejes hace que coords
    // no-grid-aligned salten a 0/16/etc al primer drag aunque el dev
    // no las haya movido.
    const f32 snap = static_cast<f32>(m_hammerSnapStep);
    ...
    const glm::vec3 effDeltaWorld = pivotWorldNew - pivotWorldStart;
    const glm::mat3 rotInv = glm::inverse(glm::mat3(worldMat));
    ...
    // Reseteamos desde `before` para idempotencia (cada frame parte del
    // estado pre-drag).
    ...
    // F2H30 fix: REBASEAR brush al nuevo centroide (mismo patron que
    // F2H12 snapshotResultWorld). Sin rebasing, el origen del entity
    // (gizmo) queda en el centro VIEJO mientras la geometria se movio.
    // Calculamos new centroid en local, lo restamos a cada
    // plane.distance, y trasladamos tf.position por R * newCentroid
    // (preserva la posicion world del mesh).
    const glm::vec3 newCentroidLocal = bc.brush.localAabb.center();
    for (auto& f : bc.brush.faces) {
        f.plane.distance += glm::dot(f.plane.normal, newCentroidLocal);
    }
    bc.brush.localAabb = Csg::computeBrushAabb(bc.brush);
    const glm::mat3 rotMat(worldMat);
    const glm::vec3 newTfPos =
        m_orthoVertexEdit.tfPosBefore + rotMat * newCentroidLocal;
    ...
    // Revert al before (planes + transform) antes del push (mismo patron
    // que MultiEditTransformCommand drag-edit) — execute() re-aplica el
    // after.
    for (usize i = 0; i < bc.brush.faces.size(); ++i) {
        bc.brush.faces[i].plane = m_orthoVertexEdit.planesBefore[i];
    }
