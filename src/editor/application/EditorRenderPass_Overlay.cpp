...
void EditorApplication::drawEditorScene3DOverlay(const glm::mat4& view,
    ...
    if (m_debugDraw) {
        ...
        if (m_scene) {
            ...
            // F2H83: 5 overlays F1-debug movidos a
            // `EditorRenderPass_Overlay_Debug.cpp` (Triggers / Joints /
            // ForceFields / Ragdolls / Vehicles). Mantiene este archivo
            // bajo el cap de LOC sin cambiar comportamiento.
            drawTriggersDebugOverlay(dbg);
            drawJointsDebugOverlay(dbg);
            drawForceFieldsDebugOverlay(dbg);
            drawRagdollsDebugOverlay(dbg);
            drawVehiclesDebugOverlay(dbg);
        }
    }
