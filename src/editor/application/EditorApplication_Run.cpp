...
// run() — ejecuta hasta m_running=false. Orden de fases por frame:
//   1) tickHotReload(dt)        - texturas + shaders.
//   2) tickFrameMetrics(dt,dtD) - FPS + PerfHud + VSync toggle.
//   3) processEvents + beginFrame.
//   4) m_ui.draw — construye widget tree.
//   5) pumpUiRequests             - Play/Stop, Toolbar, Project actions,
//                                    Open Map, Boolean Op, Welcome modals.
//   6) pumpSpawnAndDropRequests   - process*Request + Delete + Convert modal
//                                    + processViewportInteractions + drops.
//   7) updateCameras(dt).
//   8) tickSystems(dt)            - ForceField + Physics + Scripts +
//                                    Triggers + Dialog + Quest + Anim +
//                                    Nav + Particles + Audio.
//   9) renderSceneToViewport(dt) + endFrame.
//
// break-B3: el cuerpo unico de 560 LOC se partio a 5 metodos privados
// para que cada fase tenga su propio archivo mental. La logica y el
// orden de llamadas son identicos a la version pre-split — refactor SIN
...
// break-B3: fase de hot-reload (texturas + shaders). Ambas son mtime-
// throttled y se aplican entre frames para que ningun draw use un GLuint
// recien destruido.
void EditorApplication::tickHotReload(f32 dt) {
    // Hot-reload de texturas solicitado el frame anterior (boton Recargar
    // del Asset Browser). Se aplica aca, entre frames, para que ningun
    // draw en curso use un GLuint recien destruido.
    ...
    // Hot-reload de shaders (Hito 25 G): chequeo throttle 500ms de mtime
    // y recompila los .vert/.frag que cambiaron en disco. Iterativo y
    // atomico — un error de compilacion mantiene el shader anterior
    // funcionando.
    OpenGLShader::tickHotReload(dt);
}

// break-B3: FPS counter + Performance HUD + VSync toggle. Lee stats del
// frame ANTERIOR (m_renderer->frameStats todavia no fue reset por
// beginFrame de este frame) — el HUD muestra la carga real del frame que
// el dev acaba de ver.
void EditorApplication::tickFrameMetrics(f32 dt, f64 dtD) {
    const f32 fps = m_fpsCounter.tick(dtD);
    m_ui.setFps(fps);
    // F2H17: sync sub-mode al UI cada frame para statusbar e InspectorPanel.
    ...
    // F2H2: pasar metricas al Performance HUD.
    ...
    if (m_ui.performanceHud().consumeVsyncToggleRequest(vsyncRequested)) {
        ...
        // Sync el panel con el estado real del Window (puede no haberse
        // aplicado si el driver rechazo el cambio).
        m_ui.performanceHud().setVsyncState(m_window->vsyncEnabled());
    }
    (void)dt;  // dt no se usa hoy en metrics; se reserva para futuros plots.
}
...
void EditorApplication::pumpUiRequests() {
    ...
    // Pincel se setea via la rama togglePolygonDrawMode arriba; aca solo
    // gestionamos las otras 2 transiciones — y si el dev cambia de Pincel
    // a otro tool, cancelamos el pincel.
    MapTool requestedTool;
    if (m_ui.consumeMapToolRequest(requestedTool)) {
        if (requestedTool != MapTool::Pincel && m_polyDraw.active) {
            cancelPolygonDraw();
        }
        // F2H32 Bloque B: si cambiamos de tool y habia una sesion de clip
        // activa, cancelarla (sin spawnear).
        if (requestedTool != MapTool::Clip && m_clipTool.active) {
            cancelClipTool();
        }
        // F2H32 Bloque B: hint visible si se activa Clip sin nada selecto.
        // El consumer real (confirmClipTool) chequea de nuevo y aborta si
        // la seleccion sigue vacia.
        ...
        // Si requestedTool == Pincel, requestTogglePolygonDraw ya se
        // disparo (el toolbar emite ambos requests cuando clickeas el
        // boton Pincel) y la rama de arriba lo manejo.
    }
    ...
    // Sync state: para que la top toolbar pueda highlight los botones activos.
    ...
    // Hito 15 polish: el modal Welcome puede editar la lista de recientes
    // (boton X por entrada + "Limpiar inexistentes"). Cuando hay cambios,
    // sincronizamos `m_recentProjects` con la lista del UI y persistimos
    // al `editor_state.json`.
    ...
    // Modal Welcome: click en un reciente emite path directo en lugar de
    // pasar por el dialogo pfd.
...
// break-B3: dispatch de todos los process*Request (spawners de demo +
...
void EditorApplication::pumpSpawnAndDropRequests() {
    ...
    // F2H57 Bloque C: menu contextual del panel Escena puede disparar el
    // delete sin pasar por la tecla del teclado.
    if (m_ui.consumeDeleteSelectedRequest()) {
        deleteSelectedEntity();
    }

    // F2H57 Bloque D: modal "Convertir entidad". Se abre cuando se consume
    // el request del context menu del HierarchyPanel.
    renderConvertEntityModal();

    // 2.4) Interacciones del viewport: click-to-select (perspectiva + 3
    //      orthos), face mode picking, polygon brush, clip tool, drag-edit
    //      / block tool / vertex-edge edit / marquee en ortos. Extraido a
    //      `EditorApplication_RunInteractions.cpp` en AUDIT-1 para mantener
    //      este archivo bajo el hard cap de 800 LOC.
    processViewportInteractions();

    // 2.5/2.6/2.7) Drops desde AssetBrowser (textura / mesh / prefab).
    // Hito 16: implementaciones movidas a DemoSpawners.cpp para mantener
    // run() legible.
...
// break-B3: tick de TODOS los sistemas gameplay. Orden documentado en el
// audit (mismo que pre-split):
//   ForceField -> Physics -> Scripts -> Triggers -> Dialog/Quest ->
//   Animation -> Nav -> Particles -> Audio.
// Algunos solo corren en Play (ForceField, Triggers, Dialog interaction,
// Nav). Anim/Particles/Audio corren en TODOS los modos para que el dev
// vea en vivo lo que esta editando.
void EditorApplication::tickSystems(f32 dt) {
    ...
    //      stepea la sim solo en Play Mode. Despues del input y antes de
    //      scripts — asi un script puede leer la posicion post-step.
    {
    ...
    // dispatchan on_trigger_enter/_exit a los scripts. Solo en Play Mode
    // (m_playerCharId != 0). Despues del scriptSystem para que los
    // handlers ya esten registrados.
    ...
    // triggers + busca DialogComponent en la misma entidad. Si el player
    // presiono E este frame, arranca el dialog. Tambien mantiene el HUD
    // interact_prompt sincronizado con la presencia del player en el
    // trigger.
    if (m_scene && m_assetManager && m_mode == EditorMode::Play) {
        ...
        // Si dialog ya esta activo (no recien arrancado), detectar teclas
        // 1-9 para choices + E para continueNext.
        if (!dialogStarted && Dialog::DialogSystem::isActive()) {
            ...
            // Sin dialog activo: reset prev state de digitos para que el
            // primer digit que se presione sea "just pressed".
            for (int i = 0; i < 9; ++i) m_digitPrevFrame[i] = false;
        }

        // F2H52 Bloque C: ItemPickupSystem. Consume el mismo flanco de E
        // que el dialog system (si hay dialog activo, el pickup sistema lo
        // skipea para no robarle la tecla). Engine-grade: el motor SOLO
        // mueve items entre containers + dispara el hook on_pickup
        // (registrado por Lua en Bloque F).
        {
        ...
        // objectives via el LuaEvaluator (inyectado por setupQuestBindings
        // desde ScriptSystem). Lo corremos despues del ItemPickupSystem
        // para que un Collect predicate (`inventory.count(...) >= N`) que
        // recien quedo satisfecho por el pickup de este frame se complete
        // inmediatamente. Tambien despues del DialogInteractSystem para
        // que objectives Talk/Reach que dependen de `dialog.has_var(...)`
        // resuelvan en el mismo frame que el dialog set la var.
        {
            MOOD_PROFILE_SCOPE("QuestSystem::tick");
            Mood::Quest::QuestSystem::tick(*m_assetManager);
        }

        // F2H52 Bloque H: Tab togglea el widget `inventory_panel`. Flanco
        // up->down (no spam mientras se mantiene). Skipea si hay un dialog
        // abierto (Tab le pertenece al dialog si en el futuro lo usa para
        // skip).
        {
        ...
        // F2H53 Bloque G: J togglea el widget `quest_log_panel`. Mismo
        // patron que Tab para el inventory. Convencion RPG (Skyrim,
        // Witcher) — J de "Journal".
        {
    ...
    //       agentes no se mueven (mismo criterio que la fisica). Antes
    //       del NavSystem actualizamos el `target` de cada agente a la
    //       posicion del FpsCamera — los enemigos persiguen al jugador.
    //       Si en el futuro un script Lua quiere setear otro target
    //       (patrol, idle, item), lo va a poder hacer via binding cuando
    //       se exponga NavAgent a sol2.
    ...
    //       Sin pausa especifica — si el dev quiere congelarlas, baja el
    //       emitRate a 0 desde el Inspector.
    if (m_scene && m_particleSystem) {
        MOOD_PROFILE_SCOPE("ParticleSystem::update");
        m_particleSystem->update(*m_scene, dt);
    }

    // 3.6) Audio: arranca playOnStart, sincroniza posicion 3D, y setea el
    //      listener a la camara activa. Despues de scripts para que
    ...
    if (m_scene && m_audioSystem) {
        ...
        if (m_mode == EditorMode::Play) {
            ...
            // Editor Mode: listener en la posicion de la orbit cam mirando
            // al origen (target por defecto del EditorCamera).
