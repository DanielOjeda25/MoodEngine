...
// bloque de sample material) debe quedar identico al pbr.frag para que
// el grafo herede automaticamente las mejoras del PBR.
//
...
// F2H62: tiempo global para animaciones del shader graph. Lo setea
// SceneRenderer cada frame (Bloque E). Por default 0.0 si no se cablea.
uniform float uTime;
...
// F2H64: devuelve vec4 (shadowFactor + tint.rgb). Mismo contrato que pbr.frag.
...
void main() {
    // --- Sample del material via Shader Graph ---
    // Declaraciones SSA del grafo (variables intermedias del walk
    // topologico). Si el grafo esta vacio, este bloque queda vacio
    // y los 5 slots usan literals del OutputPBR.
__SHADERGRAPH_DECLS__
    ...
    // El input "normal" del OutputPBR esta declarado pero IGNORADO en v1
    // (vWorldNormal del vertex shader es la fuente de verdad). Normal
    ...
    // AO del grafo no esta soportado v1; se mantiene el uniform clasico.
    float ao = uAoMult;
    if (uHasAoMap == 1) {
    ...
    if (uDirectional.enabled == 1) {
        ...
        // F2H64: sampleShadow devuelve vec4 (factor + tint).
    ...
    // Sumar emissive del grafo al lighting final (additivo, no recibe
